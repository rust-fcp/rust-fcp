...
/// `wrap_messages`).
pub fn new_from_raw_content(path: ForwardPath, content: Vec<u8>, handle: Option<TheirSessionHandle>) -> SwitchPacket {
...
pub fn make_reply<PeerId: Clone>(replied_to_packet: &SwitchPacket, reply_content: Vec<u8>, inner_conn: &CAWrapper<PeerId>) -> SwitchPacket {
    ...
    new_from_raw_content(path, reply_content, inner_conn.peer_session_handle().map(SessionHandle).map(TheirSessionHandle))
}
